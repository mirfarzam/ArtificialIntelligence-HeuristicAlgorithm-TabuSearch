...
for i in range(0, numberOfTests):
    process = subprocess.Popen(['./algo_tabou.exe', '10', tabuDuration, numberOfCities, 'distances_entre_villes_{}.txt'.format(numberOfCities)],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    stdout, stderr = process.communicate()
    result = stdout
    result = re.sub(r'\s', ' ', str(result))
    solution = (re.findall(r'([0-9]{4-5}) km', result))[-1]
